...
| Go | wrap native [Go 1.26 `go fix`](https://go.dev/blog/gofix) | Root modules with an explicit language directive acquire checksums and a private vendor tree under explicit network authorization, then run bounded `go fix` and `go test` stages offline. Nested modules and implicit language upgrades are rejected. |
...
| Next.js | partial wrap of [official codemods](https://nextjs.org/docs/app/guides/upgrading/codemods) | Integrity-pinned codemods run serially with networking disabled; deterministic dependency edits and typed residual gates precede an explicit one-lockfile install/build/test acceptance boundary. |
