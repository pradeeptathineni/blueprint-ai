...
def test_generated_test_failure_remains_a_product_finding(python_project: Path) -> None:
    ...
    assert after.status == "partial"  # Product failure remains; test completeness was not assessed.
    assert after.assessment == "partial"
    assert any(finding.message == "product assertion failed" for finding in after.findings)
